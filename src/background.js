...
const OUATH_URL = `https://oauth.yandex.ru/authorize?response_type=token&client_id=${OAUTH_APP_ID}`;
const URL_WITH_ACCESS_TOKEN_REGEX =
  "https:\\/\\/music\\.yandex\\.(?:ru|com|by|kz|ua)\\/#access_token=([^&]*)";

let _oauthTabId;

const openOauthTab = () => {
  chrome.tabs.create({ url: OUATH_URL }, (tab) => {
    _oauthTabId = tab.id;
  });
};
